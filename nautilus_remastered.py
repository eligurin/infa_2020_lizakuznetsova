...
def star(number_of_rays, r_1, r_2, x_star_0, y_star_0):
    """
    Функция рисует Солнце, с заданными координатами, радиусами и числом лучей
    :param number_of_rays: число лучей звезды
    :param r_1: внутренний радиус
    :param r_2: внешний радиус
    :param x_star_0: координата центра звезды по оси x
    :param y_star_0: координата центра звезды по оси y
    :return:
    """
    angle = 0
    list_of_star = []
...
def cloud(x_cloud, y_cloud, x_cloud_size, y_cloud_size):
    """
    Рисует облачко с заданным положением и размерами по x и y, относительно стандартного
    :param x_cloud: координата облака по x
    :param y_cloud: координата облака по y
    :param x_cloud_size: растяжение по оси x
    :param y_cloud_size: растяжение по оси y
    :return:
    """
...
def ship(ship_size, x_ship, y_ship):
    """
    Рисует корабль с заданными координатами, подобный стандартному кораблику
    с некоторым коэффициентом подобия
    :param ship_size: коэффицциент подобия
    :param x_ship: координата кораблика по x
    :param y_ship: координата кораблика по y
    :return:
    """
    # Корпус кораблика
...
def umbrella(x_umb_size_1, x_umb_size_2, y_umb_size, x_umb, y_umb):
    """
    Рисует зонтик с заданным растяжением столба по двум осям и
    растяжением непосредественно покрытия зонта по оси x и заданными
    координатами макушки зонта
    :param x_umb_size_1: растяжение столба по оси x
    :param x_umb_size_2: растяжение покрытия зонта по оси x
    :param y_umb_size: растяжение всего зонта по оси y
    :param x_umb: координата макушки зонта по x
    :param y_umb: координата макушки зонта по y
    :return:
    """
