...
'''Stars' settings'''
(star_list, number_of_stars) = ([], int(input('Введите число звёзд:')))
for i in range(number_of_stars):
    star_list.append(list(map(int, input('Введите число лучей, внутр и внеш радиусы, коорд.центра звезды номер '
                                         + str(i + 1)
                                         + ':'
                                         ).split())))

'''Ships' settings'''
(ship_list, number_of_ships) = ([], int(input('Введите число кораблей:')))
for i in range(number_of_ships):
    ship_list.append(list(map(int, input('Введите размер и координаты кораблика номер '
                                         + str(i+1)
                                         + ':'
                                         ).split())))
'''Clouds' settings'''
(cloud_list, number_of_clouds) = ([], int(input('Введите число облаков:')))
for i in range(number_of_clouds):
    cloud_list.append(list(map(float, input('Введите размеры и координаты облака номер '
                                            + str(i+1)
                                            + ':'
                                            ).split())))
'''Umbrellas' settings'''
(umbrella_list, number_of_umbrellas) = ([], int(input('Введите число зонтов:')))
for i in range(number_of_umbrellas):
    umbrella_list.append(list(map(float, input('Введите размеры и координаты зонта номер '
                                               + str(i+1)
                                               + ':'
                                               ).split())))
pg.init()
...
for k in range(number_of_stars):
    angle = 0
    list_of_star = []
    number_of_rays = star_list[k][0]
    r_1 = star_list[k][1]
    r_2 = star_list[k][2]
    x_star_0 = star_list[k][3]
    y_star_0 = star_list[k][4]
...
'''Clouds'''
for k in range(number_of_clouds):
    x_cloud = cloud_list[k][2]
    y_cloud = cloud_list[k][3]
    x_cloud_size = cloud_list[k][0]
    y_cloud_size = cloud_list[k][1]
...
'''Ships'''
for k in range(number_of_ships):
    '''Body'''
    ship_size = ship_list[k][0]
    x_ship = ship_list[k][1]
    y_ship = ship_list[k][2]
    ...
    circle(screen, black, (x_ship + 91 * ship_size,
           ...
           ship_size)
'''Umbrellas'''
for k in range(number_of_umbrellas):
    x_umb_size_1 = umbrella_list[k][0]
    x_umb_size_2 = umbrella_list[k][1]
    y_umb_size = umbrella_list[k][2]
    x_umb = umbrella_list[k][3]
    y_umb = umbrella_list[k][4]
